...
func (mp *Multiplex) handleIncoming() {
	...
	for {
		...
		case newStreamTag:
			if ok {
				log.Printf("received NewStream message for existing stream: %v", ch)
...
func (mp *Multiplex) sendResetMsg(header uint64, hard bool) {
	...
	if err != nil && !mp.isShutdown() {
		if hard {
			log.Printf("error sending reset message: %s; killing connection", err.Error())
			mp.Close()
		} else {
			log.Printf("error sending reset message: %s", err.Error())
		}
	}
}
